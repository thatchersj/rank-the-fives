...
def _apply_inactivity(state: dict, current_year: int) -> None:
    """Update uncertainty for inactive periods.

    Elo is represented as Skill + Form. We do NOT mean-revert Skill/Form here.
    Instead, we:
      - increase sigma during inactive years (less certainty)
      - update last_year
    Form decay is handled separately using tournament time deltas.
    """
    ...
    if gap <= 0:
        state["last_year"] = int(current_year)
        return
    s = float(state.get("sigma", ELO_SIGMA_NEW))
    for _ in range(gap):
...
def compute_elo_v2(matches: dict) -> tuple[pd.DataFrame, dict]:
    ...
    This version uses a two-component model:
      Elo = Skill + Form

    - Skill moves slowly and represents long-run ability.
      Each update adds ELO_SKILL_SHARE of the Elo delta to Skill.
    - Form moves quickly and represents current momentum/edge.
      The remaining (1 - ELO_SKILL_SHARE) of each delta goes to Form.
      Form decays toward 0 with half-life ELO_FORM_HALFLIFE_YEARS.

    Returns:
      elo_df: Initial, Surname, Elo, EloSigma, (and debug columns EloSkill, EloForm)
      snapshots: dict keyed by tournament key (e.g. '19 N') with rows incl EloRank
    """
    players: dict[str, dict] = {}

    def _t_index(year: int, comp: str) -> float:
        # Approximate within-year spacing using competition order.
        # Northern / Kinnaird / London => 0/3, 1/3, 2/3.
        return float(year) + (float(COMP_ORDER.get(comp, 99)) / 3.0)

    def _decay_form(st: dict, t_idx: float) -> None:
        last_t = st.get("last_t")
        if last_t is None:
            st["last_t"] = float(t_idx)
            return
        dt = float(t_idx) - float(last_t)
        if dt <= 0:
            st["last_t"] = float(t_idx)
            return
        # Half-life decay toward 0
        hl = float(ELO_FORM_HALFLIFE_YEARS)
        if hl <= 0:
            st["last_t"] = float(t_idx)
            return
        factor = 2.0 ** (-dt / hl)
        st["form"] = float(st.get("form", 0.0)) * factor
        st["last_t"] = float(t_idx)

    def _rating(st: dict) -> float:
        return float(st.get("skill", ELO_MU)) + float(st.get("form", 0.0))

    def _apply_delta(st: dict, delta: float) -> None:
        a = float(ELO_SKILL_SHARE)
        st["skill"] = float(st.get("skill", ELO_MU)) + (a * float(delta))
        st["form"] = float(st.get("form", 0.0)) + ((1.0 - a) * float(delta))

    def get_state(pkey: str, year: int, comp: str):
        if pkey not in players:
            ...
            players[pkey] = {
                ...
                "sigma": ELO_SIGMA_NEW,
                "last_year": int(year),
                "last_t": _t_index(int(year), comp),
                "Initial": ini,
                "Surname": sur,
            }
        st = players[pkey]
        # Decay Form by elapsed tournament time, and increase sigma by inactive years.
        _decay_form(st, _t_index(int(year), comp))
        _apply_inactivity(st, int(year))
        return st

    def update_pair(win_keys, lose_keys, score: str, round_code: str, year: int, comp: str):
        coeff = 1.0
        sl = (score or "").lower()
        ...
        # Pair ratings are sums of individual total ratings
        r_w = _rating(st_w[0]) + _rating(st_w[1])
    ...
    for year, _, tkey, tinfo in tourn_list:
        ...
        nq_players: set[str] = set()
        main_players: set[str] = set()
        ...
        # Apply match updates in round order
        ms = list(tinfo.get("matches", []))
        ms.sort(key=lambda m: round_rank.get(m.get("round"), 99))
        for m in ms:
            w = m.get("winners", [])
            l = m.get("losers", [])
            if not (isinstance(w, list) and isinstance(l, list) and len(w) == 2 and len(l) == 2):
                continue
            update_pair(w, l, m.get("score", ""), m.get("round", ""), year, comp)
        ...
        for pk, st in players.items():
            rows.append({
                ...
                "Elo": round(_rating(st), 3),
                "EloSigma": round(float(st.get("sigma", ELO_SIGMA_NEW)), 3),
                "EloSkill": round(float(st.get("skill", ELO_MU)), 3),
                "EloForm": round(float(st.get("form", 0.0)), 3),
            })
        ...
        snapshots[tkey] = {"key": tkey, "rows": snap_df.to_dict(orient="records")}

    # Latest ratings dataframe
    rows = []
    for pk, st in players.items():
        rows.append({
            "Initial": st.get("Initial", ""),
